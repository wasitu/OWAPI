...
class APIComponent(ContainerComponent):
    ...
    async def start(self, ctx):
        self.add_component('kyoukai', KyoukaiComponent, ip="0.0.0.0", port=int(os.environ['PORT']), app="app:app", template_renderer=None)
        ...
        if app.config["owapi_use_redis"]:
            ...
            if 'REDIS_URL' in os.environ:
                ...
                self.add_component('redis', RedisComponent, address=url.hostname, port=url.port, password=url.password)
            else:
